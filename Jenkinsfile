pipeline {
  ...
  stages {
    stage('Status') {
      steps {
        sh 'liquibase status --url="$env:TEST_URL" --changeLogFile=$env:changeLogFile --username=postgres --password=fellaini'
        sh 'liquibase status --url="$env:QA_URL" --changeLogFile=$env:changeLogFile --username=postgres --password=fellaini'
      }
    }
    stage('test') {
       steps {
