  ...
  stages {
    ...
    stage('QA status check & update') {
        ...
        steps {
           ...
           sh 'liquibase update --url="jdbc:postgresql://database-2.cpuc6bgspxr2.eu-central-1.rds.amazonaws.com:5432/postgres" --changeLogFile=dev_wrapper.xml --username=$qa_db_USR --password=$qa_db_PSW'
       }
    }
