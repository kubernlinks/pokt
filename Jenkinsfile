pipeline {
  ...
  stages {
    ...
    stage('dev status check & update') {
      ...
      steps {
        sh 'liquibase status --url="jdbc:postgresql://database-1.cpuc6bgspxr2.eu-central-1.rds.amazonaws.com:5432/postgres" --changeLogFile=changelog_version.xml --username=${dev_db} --password=${dev_db_PSW}'
        //sh 'liquibase update --url="jdbc:postgresql://database-1.cpuc6bgspxr2.eu-central-1.rds.amazonaws.com:5432/postgres" --changeLogFile=dev_wrapper.xml --username=$dev_db_USR --password=$dev_db_PSW'   
      }
    }
